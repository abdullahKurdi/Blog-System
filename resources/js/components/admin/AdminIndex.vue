            ...
            <table class="table table-striped table-hover">
                ...
                <tbody v-if="posts">
                    <tr v-for="(post , index) in posts.data.data" :key="index">
						<td>
							<span class="custom-checkbox">
								<input type="checkbox" :id="'checkbox1'+index" name="options[]" value="1" @click.stop="selectPost(post,$event)">
                        ...
                        <td>
                            <a href="#editPostModal" class="edit" @click="editPost(post,$event)" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
			...
			<div class="modal-content">
				<form>
					...
					<div class="modal-body">
						...
						<p class="text-warning"><small><strong>Selected Posts :</strong> {{selectedPosts.length}} </small></p>
					</div>
					<div class="modal-footer">
						<input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
    ...
    methods:{
        getPosts:function (page = 1){
                ...
                .then(res => {
                    ...
                    this.posts = res.data;
                    localStorage.setItem('admin-posts',JSON.stringify(this.posts))
                })
        ...
        addPost(){
            ...
            axios.post('/api/admin/add-post',formdata,config)
            .then(res => {
                console.log(res.data)
                this.title='';
                this.body='';
        ...
        selectAll(event){
            if(event.target.checked){
                ...
                this.posts.data.data.forEach(p=>{
        ...
        deletePost(){
            axios.post('/api/admin/delete-posts',{posts_ids : this.selectedPosts})
            .then(res=>{console.log(res);$('#deletePostModal').modal('hide');this.getPosts();$('input[type="checkbox"]').prop('checked',false)})
            .catch(err => console.log(err))
        }
    },
    components:{
